...
def Report(token, guild_id1, channel_id1, message_id1, reason1):
    Responses = {
            '401: Unauthorized': f'{Fore.BLUE}Invalid Discord token.',
            'Missing Access': f'{Fore.BLUE}Missing access to channel or guild.',
            'You need to verify your account in order to perform this action.': f'{Fore.BLUE} Unverified.'
    }
    ...
    if (status := report.status_code) == 201:
        ...
        print(f"{Fore.BLUE}Error: {report.text} | Status Code: {status}\n")